...
class AddLocalVC: UIViewController,UISearchBarDelegate {
    ...
    override func viewDidLoad() {
        ...
        checkMaxLength(textField:NameTextField, maxLength: 25)
    }
    ...
    func txtfieldPlaceholder() {
        ...
        OccupationTextField.attributedPlaceholder = NSAttributedString(string: "Occupation", attributes: [
            .foregroundColor: hexStringToUIColor(hex: "#AFAFAF"),
            .font: UIFont(name:"SegoeUI-Semibold", size: 18) as Any
            ])
    ...
    func handleNotificationKeyboard(){
        NotificationCenter.default.addObserver(self, selector: #selector(AddLocalVC.keyboardWillShow), name: UIResponder.keyboardWillChangeFrameNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(AddLocalVC.keyboardWillHide), name: UIResponder.keyboardWillHideNotification, object: nil)
    }
    ...
    //MARK: ADD TAP GESTURE TO IMAGE
    func addTapToImg() {
        let tap :UITapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(profileImgTapped))
        localProfileImg.isUserInteractionEnabled = true
        localProfileImg.addGestureRecognizer(tap)
    }
    ...
    @IBAction func ContacTxtFldAct(_ sender: Any) {
    ...
    @IBAction func doneBtnAction(_ sender: Any) {
        guard let localName = NameTextField.text else{return}
        ...
        //  if localProfileImg.image == UIImage(named: "image_placeholder"){
        //   PresentAlert(message: "please choose an image.", title: "TransLife")        }
        if localName == "" || Address1 == "" || Address2 == "" || occupation == "" {
        ...
        let okAction = UIAlertAction(title: "submit", style: UIAlertAction.Style.default) {
            ...
            let dict = ["name":localName,"Address1":Address1,"Address2":Address2,"Address3":Address3,"Contact":Contact,"occupation":occupation,"DescriptionText":DescriptionText,"ProfileImageUrl": "url","LocalRating":self.ratingStackView.starsRating,"currentTimeStamp":currentTimestamp,"latitude":self.latitude,"longitude":self.longitude] as [String : Any]
            self.local.AddLocal(Dict: dict, view: self, completionHandler: { (dataRef) in
                if  let snapKey = dataRef.key {
                    self.local.saveLocalProfileImgToStorage(ImgId: snapKey , image: self.localProfileImg.image ?? #imageLiteral(resourceName: "mood_selecctor_placeholder"), view: self, completion: { (metadata) in
                        let storageRef = Storage.storage().reference().child("LocalProfileImages/\(snapKey).png")
                        print(storageRef)
                        storageRef.downloadURL(completion: { (imgUrl, error) in
                            if let error = error{
                                ...
                                ref.child("Locals").child("\(snapKey)").updateChildValues(["ProfileImageUrl" : imgUrl?.absoluteString ?? "did't get url","snapKey":snapKey])
                                if let currentuser = UserDefaults.standard.value(forKey: "username") as? String{
                                    ref.child("Locals").child(snapKey).child("usersGivenRating").updateChildValues([currentuser : self.ratingStackView.starsRating])
                                }
                                SVProgressHUD.dismiss()
                            }
                        })
                    ...
                    self.PresentAlert(message: "ERROR!", title: "TransLife")
                }
            })
...
extension AddLocalVC : CLLocationManagerDelegate {
...
extension AddLocalVC:UITableViewDataSource{
...
extension AddLocalVC:UITableViewDelegate{
...
extension AddLocalVC:UINavigationControllerDelegate,UIImagePickerControllerDelegate{
    ...
    @objc func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String : Any]) {
        ...
        if let selectedImage = selectedImageFromPicker {
            self.localProfileImg.image = selectedImage
            self.localProfileImg.contentMode = .scaleToFill
...
extension AddLocalVC:UITextViewDelegate,UITextFieldDelegate{
    //MARK:HANDLE TEXT VIEW
    func textViewDidBeginEditing(_ textView: UITextView) {
        if DescriptionTextView.text == "Use this space to describe local..."{
    ...
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        ...
        }else if OccupationTextField.isEditing == true{
